...
void ghost_draw(Ghost* ghost) {
	...
	//Draw default image
	if (ghost->status == FREEDOM || ghost->status == BLOCKED || ghost->status == GO_OUT) {
		al_draw_scaled_bitmap(ghost->move_sprite,
				((ghost->objData.facing <= 0 ? 1 : ghost->objData.facing) - 1) * 32 + (ghost->objData.moveCD % FPS < FPS / 2 ? 0 : 1) * 16, 0,
				16, 16,
	...
	*/
	...
	// [TODO] below is for animation usage, change the sprite you want to use.
		...
		al_draw_scaled_bitmap(ghost->flee_sprite, (ghost->objData.moveCD % FPS < FPS / 2 ? 0 : 1) * 16, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
		...
		al_draw_scaled_bitmap(ghost->dead_sprite, (ghost->objData.facing-1) * 16, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	Directions PreMove = ghost->objData.preMove;
	int nxt = (shortestDirection % 2 == 0 ? shortestDirection-1 : shortestDirection+1);
	int nxt2 = (PreMove % 2 == 0 ? PreMove-1 : PreMove+1);
	// Description:
	...
	int cnt = 0;
	bool flagNxt = false, flagNxt2 = false, flagPre = false;
	for (Directions i = 1; i <= 4; i++) {
		if (ghost_movable(ghost, M, i, false)) {
			proba[cnt++] = i;
			if (i == nxt) flagNxt = true;
			if (i == nxt2) flagNxt2 = true;
			if (i == PreMove) flagPre = true;
		}
	}
	...
	if (generateRandomNumber(1, 100) < possibility) {
		if (flagPre) nxt_dir = PreMove;
		else if (flagNxt) nxt_dir = nxt;
		else nxt_dir = proba[generateRandomNumber(0, cnt-1)];
	}
	else {
		if (flagNxt2) nxt_dir = nxt2;
		else nxt_dir = proba[generateRandomNumber(0, cnt - 1)];
	}
	game_log("Choose %d dir\n", nxt_dir);
	ghost_NextMove(ghost, nxt_dir);
}
