	...
	if (game_over) {
		...
		*/
		if (!al_get_timer_started(pman->death_anim_counter)) {
			game_abort("Pacman Death Timer not started!");
		}
		...
		al_draw_scaled_bitmap(pman->die_sprite, 16 * ((al_get_timer_count(pman->death_anim_counter) / 6 > offset ? offset++ : offset) % 14), 0, 16, 16,
		...
		al_draw_scaled_bitmap(pman->move_sprite,
			((pman->objData.facing <= 0 ? 1 : pman->objData.facing) - 1) * 32 + (pman->objData.moveCD % FPS < FPS / 2 ? 0 : 1) * 16, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
...
void pacman_die() {
	stop_bgm(PACMAN_MOVESOUND_ID);
	//al_set_sample(PACMAN_DEATH_SOUND_INSTANCE, PACMAN_DEATH_SOUND);
	PACMAN_MOVESOUND_ID = play_audio(PACMAN_DEATH_SOUND, effect_volume);
}

void pacman_eat_ghost(void)
{
	stop_bgm(PACMAN_MOVESOUND_ID);
